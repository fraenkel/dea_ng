...
type StagingTaskWorkspace struct {
	baseDir               string
	adminBuildpacks       []string
	buildpacksInUse       []string
	environmentProperties map[string]string
}

func NewStagingTaskWorkspace(baseDir string, adminBuildpacks []string, buildpacksInUse []string, environmentProperties map[string]string) StagingTaskWorkspace {
	...
	os.MkdirAll(s.adminBuildpacksDir(), 0755)
	os.MkdirAll(s.workspaceDir(), 0755)
	return s
}

func (s StagingTaskWorkspace) tmpDir() string {
	return path.Join(s.baseDir, "tmp")
}

func (s StagingTaskWorkspace) adminBuildpacksDir() string {
	return path.Join(s.baseDir, "admin_buildpacks")
}

func (s StagingTaskWorkspace) workspaceDir() string {
	return path.Join(s.baseDir, "staging")
}

func (s StagingTaskWorkspace) staging_info_path() string {
	return path.Join(s.workspaceDir(), STAGING_INFO)
}
