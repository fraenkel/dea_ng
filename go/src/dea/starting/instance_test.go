...
func TestDefaultAttributes(t *testing.T) {
	instance := NewInstance(validInstanceAttributes(false), nil)
...
func TestMissingStartCommand(t *testing.T) {
	instance := NewInstance(map[string]interface{}{}, nil)
	assert.Equal(t, instance.StartCommand(), "")
}

func TestOtherAttributes(t *testing.T) {
	limits := map[string]int{"mem": 1, "disk": 2, "fds": 3}
	services := []map[string]interface{}{map[string]interface{}{"name": "redis", "type": "redis"}}
	instance := NewInstance(map[string]interface{}{
		"limits":   limits,
		"env":      []string{"FOO=BAR", "BAR=", "QUX"},
		"services": services,
	}, nil)

	assert.Equal(t, instance.Limits(), limits)
	assert.Equal(t, instance.Environment(), map[string]string{"FOO": "BAR", "BAR": "", "QUX": ""})
	assert.Equal(t, instance.Services(), services)
}

func TestMemoryLimit(t *testing.T) {
	instance := NewInstance(validInstanceAttributes(false), nil)
	assert.Equal(t, instance.MemoryLimit(), 512*1024*1024)
}

func TestDiskLimit(t *testing.T) {
	instance := NewInstance(validInstanceAttributes(false), nil)
	assert.Equal(t, instance.DiskLimit(), 128*1024*1024)
}

func TestFileDescriptorLimit(t *testing.T) {
	instance := NewInstance(validInstanceAttributes(false), nil)
	assert.Equal(t, instance.FileDescriptorLimit(), 5000)
}

func validInstanceAttributes(lotsOfServices bool) map[string]interface{} {
	attrs := map[string]interface{}{
		...
		"limits": map[string]int{"mem": 512, "disk": 128, "fds": 5000},
		"env":    []string{"FOO=BAR"},
	}

	services := []map[string]interface{}{validServiceAttributes("")}
	if lotsOfServices {
		services = append(services, validServiceAttributes("syslog://log.example.com"))
		services = append(services, validServiceAttributes("syslog://log2.example.com"))
	}
...
func validServiceAttributes(syslogDrainUrl string) map[string]interface{} {
